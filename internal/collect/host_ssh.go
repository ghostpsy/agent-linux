...
func CollectHostSSH() (*payload.HostSSH, string) {
	output, err := runSSHDT()
	if err != nil {
		return nil, collectionNote("OpenSSH effective configuration could not be read from sshd.")
	}
...
func normalizeListenAddresses(addresses []string) []string {
	...
	for _, address := range addresses {
		...
		seen[value] = struct{}{}
		normalized = append(normalized, truncateRunes(value, 128))
	}
	return normalized
}
...
func truncateList(values []string, maxItems, maxItemRunes int) []string {
	...
	for _, value := range values {
		...
		out = append(out, truncateRunes(value, maxItemRunes))
	}
	return out
}
