...
`
	...
	out := collectApacheHttpdPostureWithBinary(context.Background(), bin, bin, nil)
...
`
	...
	defer cancel()
	out := collectApacheHttpdPostureWithBinary(ctx, bin, bin, nil)
...
`
	...
	if !strings.Contains(out.Version, "Apache/2.4.test") {
