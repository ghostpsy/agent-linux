...
func collectSystemdServices(parent context.Context) ([]payload.ServiceEntry, string) {
	...
	if err != nil {
		...
		return nil, shared.CollectionNote("systemd D-Bus connection failed.")
	}
	defer conn.Close()
	...
	for _, u := range units {
		...
		se := payload.ServiceEntry{
			Name:        name,
			Manager:     "systemd",
			ActiveState: shared.TruncateRunes(activeState, 32),
		}
		if st, ok := fileState[name]; ok {
			se.UnitFileState = shared.TruncateRunes(st, 32)
...
func collectFromServiceStatusAll(parent context.Context, serviceBin string) ([]payload.ServiceEntry, string) {
	...
	if err != nil && len(strings.TrimSpace(string(combined))) == 0 {
		...
		return nil, shared.CollectionNote("service --status-all did not return usable output.")
	}
	...
	for _, name := range names {
		...
		name = shared.TruncateRunes(strings.TrimSpace(name), 256)
		if name == "" {
			continue
		}
		se := payload.ServiceEntry{
...
func collectFromChkconfig(parent context.Context) ([]payload.ServiceEntry, string) {
	...
	if err != nil && len(strings.TrimSpace(string(combined))) == 0 {
		...
		return nil, shared.CollectionNote("chkconfig --list did not return usable output.")
	}
	rows := parseChkconfigList(combined)
	if len(rows) == 0 {
		return nil, shared.CollectionNote("chkconfig did not report any SysV services.")
	}
...
func parseChkconfigList(output []byte) []payload.ServiceEntry {
	...
	for sc.Scan() {
		...
		name := shared.TruncateRunes(strings.TrimSpace(fields[0]), 256)
		if name == "" {
			continue
		}
